  ...
  try {
    ...
    const response = {
      ...
      records: records.map(record => ({
        ...
        abs_und_wp: record.status === 'Approved' && record.leave_type === 'Sick Leave' ? 'Yes' : 'No', // Example logic
        balance: balances.find(b => b.leave_type === record.leave_type)?.balance || 0, // Fetch balance for the specific leave type
